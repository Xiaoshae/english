...
**状语从句**是复杂句中用于**修饰主句动词、形容词或整个主句的从句**，通过**从属连词**（如when, if, because等）与主句连接，表达时间、条件、原因、目的、让步等逻辑关系。



从生成语法（Generative Syntax）的角度看，状语从句是**附加语（adjunct）**，通过 **CP（Complementizer Phrase）** 结构嵌入主句，其核心功能是为句子提供**附加信息**（如时间、条件、原因等）。



**状语从句的本质**是在句中充当**副词性成分**，回答以下问题：

- **When?**（时间）
- **Why?**（原因）
- **Under what condition?**（条件）
- **For what purpose?**（目的）
- **Despite what?**（让步）
...
| **类型**         | **常见引导词**                 | **核心语义**             |
| ---------------- | ------------------------------ | ------------------------ |
| **时间状语从句** | when, while, before, after, as | 事件发生的时间关系       |
| **条件状语从句** | if, unless, provided that      | 假设或前提               |
| **原因状语从句** | because, since, as             | 解释主句动作的理由       |
| **目的状语从句** | so that, in order that         | 主句动作的目标或意图     |
| **让步状语从句** | although, even though, though  | 主句动作与预期相反的情况 |
| **结果状语从句** | so...that, such...that         | 主句动作导致的后果       |



通过**从属连词插入**和**语序调整**，将逻辑关系转化为合法句子：

1. 确定主句与从句的逻辑关系（如条件、时间）；
2. 选择对应的从属连词（如if, when）；
3. 将从句嵌入主句，调整语序。

示例：

**深层逻辑**：主句与条件是假设关系 → You pass the exam → I will buy you a gift.

**插入连词**：**If** you pass the exam, I will buy you a gift.



状语从句由 **从属连词 + 主语 + 谓语** 构成，可置于主句前或主句后：
